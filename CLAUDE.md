...
- **`app/api/__init__.py`** — Aggregates all routers: auth, nomenclator, cheltuieli, portofele, rapoarte, settings
...
- **`src/App.tsx`** — Routes: `/login`, `/` (Dashboard), `/cheltuieli`, `/rapoarte`, `/settings/*`. Protected routes wrap with `<Layout>`. Cheltuieli and Rapoarte pages are placeholder stubs.
...
- Key tables: `settings`, `users`, `portofele`, `categorii`, `grupe`, `nomenclator` (with 384-dim vector embedding), `exercitii` (daily accounting period), `cheltuieli` (transactions), `transferuri`, `alimentari`, `chat_history`
